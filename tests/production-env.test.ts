...
test("accepts a complete hardened production environment", () => {
  const result = inspectProductionEnvironment({ DATABASE_URL: "postgres://database", AUTH_SECRET: "a".repeat(32), CRON_SECRET: "c".repeat(32), APP_URL: "https://senzilytics.example" });
