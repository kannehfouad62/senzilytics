...
test("unified calendar recognizes terminal source statuses",()=>{for(const status of ["COMPLETED","CLOSED","CANCELLED","APPROVED","VERIFIED","REJECTED","EXPIRED"])assert.equal(isUnifiedCalendarStatusComplete(status),true)});
