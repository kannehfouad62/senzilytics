...
test("governed model outputs persist server-calculated variables and protect native exports",async()=>{const[action,workbook,presentation]=await Promise.all([readFile(new URL("../src/features/research/analysis-actions.ts",import.meta.url),"utf8"),readFile(new URL("../src/app/api/research/analyses/[analysisId]/workbook/route.ts",import.meta.url),"utf8"),readFile(new URL("../src/app/api/research/analyses/[analysisId]/presentation/route.ts",import.meta.url),"utf8")]);assert.match(action,/variableKeys/);assert.match(action,/multipleLinearRegression/);assert.match(action,/logisticRegression/);assert.match(action,/cronbachAlpha/);for(const source of[workbook,presentation]){assert.match(source,/EXPORT_RESEARCH_OUTPUTS/);assert.match(source,/organizationId/);assert.match(source,/no-store/)}});
