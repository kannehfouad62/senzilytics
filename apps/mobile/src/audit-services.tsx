...
export function AuditServicesScreen({
  ...
  return <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
    ...
    <View style={styles.actions}>
      ...
      {workspace.auditServiceCapabilities.canManage ? <Button label="Open full workspace" disabled={!online} onPress={() => void onOpenPath("/audit-services")} /> : null}
    </View>
    ...
    {engagements.map((engagement) => <EngagementCard key={engagement.id} engagement={engagement} online={online} onOpenPath={onOpenPath} />)}
...
function EngagementCard({ engagement, online, onOpenPath }: { engagement: MobileAuditServiceEngagement; online: boolean; onOpenPath: (path: string) => Promise<void> }) {
  ...
  return <View style={styles.card}>
    ...
    {engagement.externalAccesses.slice(0, 2).map((access) => <Text key={access.id} style={styles.signal}>External: {humanize(access.scope)} · {access.decision ? humanize(access.decision.decision) : access.findingResponse ? humanize(access.findingResponse.reviewStatus) : humanize(access.status)} · {access.contact.name}</Text>)}
    <Button label={online ? "Open engagement" : "Connect to manage"} disabled={!online} onPress={() => void onOpenPath(`/audit-services/engagements/${engagement.id}`)} />
  </View>;
}
