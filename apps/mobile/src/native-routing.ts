export type NativeRecordTarget =
  ...
  | { tab: "complianceDocuments"; view: "compliance" | "documents"; recordId?: string }
  | { tab: "controlledWork"; view: "moc" | "permits"; recordId?: string }
  | { tab: "assetContractors"; view: "assets" | "contractors"; recordId?: string }
  | { tab: "chemicalEnvironmental"; view: "chemicals" | "environmental"; recordId?: string }
  | { tab: "behaviorAssurance"; view: "behavior" | "sif" | "certification"; recordId?: string };
...
export function resolveNativeRecordTarget(link: string | null | undefined): NativeRecordTarget | null {
  if (!link) return null;
  ...
  switch (parts[0]) {
    ...
    case "compliance":
      if (parts[1] === "permits") return { tab: "controlledWork", view: "permits", recordId: recordId(parts, 2) };
    ...
    case "assurance":
      if (parts[1] === "sif") return { tab: "behaviorAssurance", view: "sif", recordId: parts.at(-1) };
      if (parts[1] === "certification") return { tab: "behaviorAssurance", view: "certification", recordId: parts.at(-1) };
      return null;
    default: return null;
  }
}

export function resolveWorkflowNativeTarget(entityType: string, entityId: string, href?: string | null) {
  const linked = resolveNativeRecordTarget(href);
  if (linked) return linked;
  const fallback: Record<string, string> = {
    INCIDENT: `/incidents/${entityId}`,
    CORRECTIVE_ACTION: `/actions/${entityId}`,
    AUDIT: `/audits/${entityId}`,
    INSPECTION: `/inspections/${entityId}`,
    COMPLIANCE: `/compliance/${entityId}`,
    DOCUMENT: `/documents/${entityId}`,
    PERMIT: `/compliance/permits/${entityId}`,
    CHEMICAL: `/chemicals/${entityId}`,
    MOC: `/moc/${entityId}`,
    OBSERVATION: `/observations/${entityId}`,
    RISK: `/risks/${entityId}`,
    TRAINING: `/training/${entityId}`,
  };
