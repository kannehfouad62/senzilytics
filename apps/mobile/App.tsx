...
import {
  ...
  type AssetContractorView,
} from "./src/assets-contractors";
import { AuditServicesScreen } from "./src/audit-services";
import {
  BehaviorAssuranceScreen,
  type BehaviorAssuranceView,
} from "./src/behavior-assurance";
...
} from "./src/compliance-documents";
import {
  ChemicalEnvironmentalScreen,
  type ChemicalEnvironmentalView,
} from "./src/chemical-environmental";
import { EsgScreen, type EsgView } from "./src/esg";
import {
  ExecutiveCommandScreen,
  type ExecutiveCommandView,
} from "./src/executive-command";
import {
  HygieneHealthScreen,
  type HygieneHealthView,
} from "./src/hygiene-health";
import { MocPermitScreen, type MocPermitView } from "./src/moc-permits";
import { OfflineOutboxScreen } from "./src/offline-outbox";
...
function SenzilyticsApp() {
  ...
  const [complianceTrainingView, setComplianceTrainingView] = useState<ComplianceTrainingView>("calendar");
  const [complianceDocumentView, setComplianceDocumentView] =
    useState<ComplianceDocumentView>("compliance");
  const [mocPermitView, setMocPermitView] = useState<MocPermitView>("moc");
  const [assetContractorView, setAssetContractorView] =
  ...
  const [esgView, setEsgView] = useState<EsgView>("disclosures");
  const [behaviorAssuranceView, setBehaviorAssuranceView] =
  ...
  const [nativeRiskId, setNativeRiskId] = useState<string | null>(null);
  ...
  const refreshWorkspace = useCallback(async () => {
    ...
    if (!next.complianceDocumentCapabilities.canManageDocuments) {
      await clearCachedControlledDocuments(nextOwner);
    }
    setWorkspace(merged);
    setVerifiedAt(Date.parse(verified));
    await cacheWorkspace(nextOwner, merged, verified);
    setPending(await pendingOfflineCount(nextOwner));
    return merged;
  }, []);

  const refreshExecutiveWorkspace = async () => {
    if (!workspace) {
      throw new Error("The mobile workspace is not available.");
    }
    const executive = await mobileApi<MobileExecutiveWorkspace>(
      "/api/mobile/executive"
    );
    const current = workspaceRef.current ?? workspace;
    const merged = { ...current, ...executive };
    const executiveOwner = `${merged.organization.id}:${merged.user.id}`;
    const verified = new Date().toISOString();
    setWorkspace(merged);
    setVerifiedAt(Date.parse(verified));
  ...
  const openNativeTarget = useCallback((target: NativeRecordTarget) => {
    switch (target.tab) {
      case "capture":
        ...
        break;
      case "actions":
        setNativeActionId(target.recordId ?? null);
  ...
  return (
    <SafeAreaView style={styles.app}>
      ...
      {tab === "risks" && <RiskFieldScreen workspace={workspace} ownerKey={ownerKey} online={online} initialView={riskFieldView} initialRecordId={nativeRiskId} onBack={() => { setNativeRiskId(null); setTab("workspace"); }} onQueued={async (message) => { setPending(await pendingOfflineCount(ownerKey)); setNotice(message); }} onSync={sync} />}
      {tab === "governance" && <ComplianceTrainingScreen workspace={workspace} ownerKey={ownerKey} online={online} initialView={complianceTrainingView} initialRecordId={nativeGovernanceId} onBack={() => { setNativeGovernanceId(null); setTab("workspace"); }} onQueued={async (message) => { setPending(await pendingOfflineCount(ownerKey)); setNotice(message); }} onSync={sync} />}
      ...
      {tab === "executive" && <ExecutiveCommandScreen workspace={workspace} online={online} initialView={executiveCommandView} initialRecordId={nativeExecutiveId} onBack={() => { setNativeExecutiveId(null); setTab("workspace"); }} onRefresh={refreshExecutiveWorkspace} onNotice={setNotice} onOpenNativeTarget={openNativeTarget} />}
...
function WorkspaceScreen({ modules, online, onCapture, onInspect, onAudit, onAuditServices, onResearch, onRisk, onGovernance, onComplianceDocuments, onControlledWork, onAssetContractor, onHygieneHealth, onChemicalEnvironmental, onEsg, onBehaviorAssurance, onRegulatory, onExecutive, onAdministration, onActions }: { modules: MobileModule[]; online: boolean; onCapture: (mode: CaptureMode) => void; onInspect: () => void; onAudit: () => void; onAuditServices: () => void; onResearch: () => void; onRisk: (view: RiskFieldView) => void; onGovernance: (view: ComplianceTrainingView) => void; onComplianceDocuments: (view: ComplianceDocumentView) => void; onControlledWork: (view: MocPermitView) => void; onAssetContractor: (view: AssetContractorView) => void; onHygieneHealth: (view: HygieneHealthView) => void; onChemicalEnvironmental: (view: ChemicalEnvironmentalView) => void; onEsg: (view: EsgView) => void; onBehaviorAssurance: (view: BehaviorAssuranceView) => void; onRegulatory: () => void; onExecutive: (view: ExecutiveCommandView) => void; onAdministration: (view: TenantAdministrationView) => void; onActions: (view: ActionCenterView) => void }) {
  ...
  return (
    ...
    >
      <Text style={styles.eyebrow}>ROLE-AWARE ACCESS</Text>
      <Text style={styles.pageTitle}>My workspace</Text>
      <Text style={styles.muted}>
        Your tenant and role determine what appears here. Every operational
        module opens inside the native Senzilytics app; authorized field
        workflows remain available offline.
      </Text>
      <Input
        value={query}
        onChangeText={setQuery}
        placeholder="Search my authorized modules"
        autoCapitalize="none"
      />
      {!online ? (
        <View style={styles.offlineBanner}>
          <Text style={styles.offlineBannerText}>
            ...
            behavior coaching, SIF critical-control verification, and
            certification readiness reviews,
            regulatory source reviews and impact assessments,
            executive dashboards, reports, assurance signals, and prior
            intelligence analyses,
            compliance obligations, permit exposure, and encrypted controlled
            document copies,
      ...
      {categories.map((category) => {
        const categoryModules = visible.filter(
          (module) => module.category === category
        );
        if (!categoryModules.length) return null;
        return (
          <View key={category} style={styles.moduleSection}>
            <Text style={styles.sectionTitle}>{humanize(category)}</Text>
            {categoryModules.map((module) => (
              <Card key={module.key} accent={Boolean(module.nativeCapability)}>
                <View style={styles.moduleHeading}>
                  <View style={styles.moduleMark}>
                    <Text style={styles.moduleMarkText}>{module.label.slice(0, 1)}</Text>
                  </View>
                ...
                <View style={styles.row}>
                  {module.nativeCapability === "ACTION_CENTER" ? <SecondaryButton label="Open native task inbox" onPress={() => onActions("tasks")} /> : null}
                  {module.nativeCapability === "CAPA_EXECUTION" ? <SecondaryButton label="Open native CAPA" onPress={() => onActions("capa")} /> : null}
                  {module.nativeCapability === "OBSERVATION_CAPTURE" ? <SecondaryButton label="Capture observation" onPress={() => onCapture("observation")} /> : null}
                  {module.nativeCapability === "INCIDENT_CAPTURE" ? <SecondaryButton label="Report incident" onPress={() => onCapture("incident")} /> : null}
                  {module.nativeCapability === "INSPECTION_EXECUTION" ? <SecondaryButton label="Assigned inspections" onPress={onInspect} /> : null}
                  {module.nativeCapability === "AUDIT_EXECUTION" ? <SecondaryButton label="Assigned Audits" onPress={onAudit} /> : null}
                  {module.nativeCapability === "AUDIT_SERVICE_DELIVERY" ? <SecondaryButton label="Audit-service delivery" onPress={onAuditServices} /> : null}
                  {module.nativeCapability === "RESEARCH_FIELDWORK" ? <SecondaryButton label="Assigned research fieldwork" onPress={onResearch} /> : null}
                  {module.nativeCapability === "RISK_FIELD" ? <SecondaryButton label="Open native Risk Register" onPress={() => onRisk("risks")} /> : null}
                  {module.nativeCapability === "JSA_FIELD" ? <SecondaryButton label="Open native JSA / JHA" onPress={() => onRisk("jsa")} /> : null}
                  {module.nativeCapability === "COMPLIANCE_REGISTER" ? <SecondaryButton label="Open native compliance" onPress={() => onComplianceDocuments("compliance")} /> : null}
                  {module.nativeCapability === "COMPLIANCE_CALENDAR" ? <SecondaryButton label="Open native calendar" onPress={() => onGovernance("calendar")} /> : null}
                  {module.nativeCapability === "TRAINING_ASSIGNMENTS" ? <SecondaryButton label="Open native training" onPress={() => onGovernance("training")} /> : null}
                  {module.nativeCapability === "MOC_EXECUTION" ? <SecondaryButton label="Open native MOC" onPress={() => onControlledWork("moc")} /> : null}
                  {module.nativeCapability === "PERMIT_TO_WORK_EXECUTION" ? <SecondaryButton label="Open native permits" onPress={() => onControlledWork("permits")} /> : null}
                  {module.nativeCapability === "ASSET_FIELD" ? <SecondaryButton label="Open native assets" onPress={() => onAssetContractor("assets")} /> : null}
                  ...
                  {module.nativeCapability === "ESG_FIELD" ? <SecondaryButton label="Open native ESG" onPress={() => onEsg("disclosures")} /> : null}
                  {module.nativeCapability === "BEHAVIOR_SAFETY_FIELD" ? <SecondaryButton label="Open native Behavior Safety" onPress={() => onBehaviorAssurance("behavior")} /> : null}
                  {module.nativeCapability === "SIF_ASSURANCE_FIELD" ? <SecondaryButton label="Open native SIF assurance" onPress={() => onBehaviorAssurance("sif")} /> : null}
                  {module.nativeCapability === "CERTIFICATION_ASSURANCE" ? <SecondaryButton label="Open native certification readiness" onPress={() => onBehaviorAssurance("certification")} /> : null}
                  {module.nativeCapability === "REGULATORY_INTELLIGENCE" ? <SecondaryButton label="Open native regulatory intelligence" onPress={onRegulatory} /> : null}
                  {module.nativeCapability === "CONTROLLED_DOCUMENTS" ? <SecondaryButton label="Open native documents" onPress={() => onComplianceDocuments("documents")} /> : null}
                  {module.nativeCapability === "EXECUTIVE_DASHBOARD" ? <SecondaryButton label="Open native executive dashboard" onPress={() => onExecutive("overview")} /> : null}
      ...
      {!visible.length ? <EmptyState text="No authorized module matches your search." /> : null}
    </ScrollView>
  );
}
...
function MobileMatrixField({ field, value, onChange }: { field: RuntimeField; value: FieldValue | undefined; onChange: (value: FieldValue) => void }) {
...
function preserveExecutiveSnapshot(
  ...
  const hasSnapshot = Boolean(
    previous?.executiveDashboard ||
      previous?.executivePortfolio ||
      previous?.operationalAssurance ||
      previous?.executiveReport ||
      previous?.executiveAiMetrics ||
      previous?.executiveAiAnalyses?.length
  );

  if (!sameOwner || !previous || !hasSnapshot) return next;

  return {
    ...next,
    executiveGeneratedAt: previous.executiveGeneratedAt,
    executiveDashboard: next.executiveCapabilities.canViewDashboard
      ? previous.executiveDashboard
      : null,
    executivePortfolio: next.executiveCapabilities.canViewDashboard
      ? previous.executivePortfolio
      : null,
    operationalAssurance: next.executiveCapabilities.canViewDashboard
      ? previous.operationalAssurance
      : null,
    executiveReport: next.executiveCapabilities.canViewReports
      ? previous.executiveReport
      : null,
    executiveAiAnalyses: next.executiveCapabilities.canUseAi
      ? previous.executiveAiAnalyses
      : [],
...
const styles = StyleSheet.create({
  ...
  blockingOverlay: { position: "absolute", top: 0, right: 0, bottom: 0, left: 0, zIndex: 20, alignItems: "center", justifyContent: "center", padding: 28, backgroundColor: "#07111ff2" }, blockingTitle: { color: "#f8fafc", fontSize: 20, fontWeight: "800", marginTop: 18 }, blockingText: { color: "#94a3b8", fontSize: 13, lineHeight: 19, marginTop: 8, textAlign: "center" },
