...
async function main() {
  ...
  const forklift = await prisma.asset.upsert({
    where: { organizationId_reference: { organizationId: organization.id, reference: "AST-DEMO-001" } },
    update: {},
    create: {
      id: "asset_public_demo_forklift",
      organizationId: organization.id,
      reference: "AST-DEMO-001",
      name: "Warehouse Forklift 01",
      description: "Fictional safety-critical mobile equipment used to demonstrate inspection, maintenance, defect, and CAPA governance.",
      type: AssetType.MOBILE_EQUIPMENT,
      status: AssetStatus.OUT_OF_SERVICE,
      criticality: RiskLevel.CRITICAL,
      isSafetyCritical: true,
      manufacturer: "Northstar Demo Equipment",
      modelNumber: "FLT-2500",
      serialNumber: "DEMO-FLT-0001",
      siteId: site.id,
      departmentId: department.id,
      location: "Warehouse staging bay",
      ownerId: manager.id,
      createdById: manager.id,
      commissionedAt: days(-900),
      inspectionIntervalDays: 7,
      lastInspectionAt: days(-9),
      nextInspectionDueAt: days(-2),
      maintenanceIntervalDays: 90,
      lastMaintenanceAt: days(-110),
      nextMaintenanceDueAt: days(-20),
      permitRequired: true,
    },
  });
  const forkliftInspection = await prisma.assetInspection.upsert({
    where: { id: "asset_inspection_public_demo_forklift" },
    update: {},
    create: {
      id: "asset_inspection_public_demo_forklift",
      organizationId: organization.id,
      assetId: forklift.id,
      inspectedById: manager.id,
      inspectedAt: days(-9),
      result: AssetInspectionResult.OUT_OF_SERVICE,
      conditionScore: 2,
      evidenceReference: "DEMO-PHOTO-FLT-01",
      observations: "Service-brake response was inconsistent during the pre-use function test.",
      immediateAction: "Equipment isolated, key controlled, and out-of-service tag applied.",
      nextInspectionDueAt: days(-2),
    },
  });
  const forkliftDefect = await prisma.assetDefect.upsert({
    where: { organizationId_reference: { organizationId: organization.id, reference: "DEF-DEMO-001" } },
    update: {},
    create: {
      id: "asset_defect_public_demo_forklift",
      organizationId: organization.id,
      assetId: forklift.id,
      sourceInspectionId: forkliftInspection.id,
      reference: "DEF-DEMO-001",
      title: "Forklift service-brake performance degraded",
      description: "Pre-use testing identified inconsistent stopping response on a safety-critical mobile asset.",
      severity: RiskLevel.CRITICAL,
      status: AssetDefectStatus.REPAIR_PLANNED,
      reportedById: manager.id,
      ownerId: manager.id,
      dueDate: days(3),
      immediateControls: "Forklift remains isolated from service.",
      repairPlan: "Authorized technician to inspect the brake circuit and complete a documented function test.",
    },
  });
  await prisma.assetMaintenanceRecord.upsert({
    where: { id: "asset_maintenance_public_demo_forklift" },
    update: {},
    create: {
      id: "asset_maintenance_public_demo_forklift",
      organizationId: organization.id,
      assetId: forklift.id,
      defectId: forkliftDefect.id,
      type: AssetMaintenanceType.CORRECTIVE,
      status: AssetMaintenanceStatus.SCHEDULED,
      title: "Inspect and repair forklift service brake",
      scheduledAt: days(1),
      dueAt: days(3),
      technicianId: manager.id,
      serviceProvider: "Fictional Authorized Equipment Service",
      workOrderReference: "WO-DEMO-1042",
    },
  });
  await prisma.asset.upsert({
    where: { organizationId_reference: { organizationId: organization.id, reference: "AST-DEMO-002" } },
    update: {},
    create: {
      id: "asset_public_demo_eyewash",
      organizationId: organization.id,
      reference: "AST-DEMO-002",
      name: "Fabrication Bay Emergency Eyewash",
      description: "Fictional emergency equipment with current inspection and maintenance controls.",
      type: AssetType.EMERGENCY_EQUIPMENT,
      status: AssetStatus.ACTIVE,
      criticality: RiskLevel.HIGH,
      isSafetyCritical: true,
      manufacturer: "Northstar Demo Safety",
      serialNumber: "DEMO-EYE-0001",
      siteId: site.id,
      departmentId: department.id,
      location: "Fabrication bay east entrance",
      ownerId: manager.id,
      createdById: manager.id,
      inspectionIntervalDays: 7,
      lastInspectionAt: days(-2),
      nextInspectionDueAt: days(5),
