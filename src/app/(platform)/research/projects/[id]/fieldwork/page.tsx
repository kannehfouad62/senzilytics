...
import { buildInterviewerQuality, detectFieldworkLocationClusters, summarizeFieldworkAssurance } from "@/modules/research/research-fieldwork-assurance";
...
export default async function ResearchFieldworkDashboard({
  ...
  return (
    <div>
      ...
      <section className="mt-6 overflow-hidden rounded-3xl border border-white/10 bg-white/[.035]">
        <div className="border-b border-white/10 p-6"><p className="text-sm text-violet-300">Quality intelligence</p><h2 className="mt-1 text-xl font-semibold">Interviewer monitoring</h2><p className="mt-2 max-w-4xl text-sm text-slate-400">These transparent indicators help supervisors prioritize coaching and verification. They are not performance scores and must not be used as automated employment decisions.</p></div>
        <div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-white/[.03] text-xs text-slate-400"><tr><th className="px-4 py-3">Enumerator</th><th className="px-4 py-3">Interviews</th><th className="px-4 py-3">Median duration</th><th className="px-4 py-3">Average sync delay</th><th className="px-4 py-3">Location coverage</th><th className="px-4 py-3">Back-checks</th><th className="px-4 py-3">Review indicators</th></tr></thead><tbody className="divide-y divide-white/10">{interviewerQuality.map((item) => <tr key={item.enumeratorId}><td className="px-4 py-4 font-medium">{item.name}</td><td className="px-4 py-4">{item.interviews}</td><td className="px-4 py-4">{item.medianDurationMinutes.toFixed(1)} min</td><td className="px-4 py-4">{item.averageSyncDelayHours.toFixed(1)}h</td><td className="px-4 py-4">{item.locationCoverage.toFixed(1)}%</td><td className="px-4 py-4"><p>{item.backchecksVerified}/{item.backchecksSelected} verified</p>{item.backchecksRejected ? <p className="text-xs text-red-300">{item.backchecksRejected} rejected</p> : null}</td><td className="px-4 py-4"><span className={`rounded-full px-2 py-1 text-xs ${item.reviewPriority ? "bg-amber-400/10 text-amber-200" : "bg-emerald-400/10 text-emerald-200"}`}>{item.reviewPriority} response{item.reviewPriority === 1 ? "" : "s"}</span></td></tr>)}</tbody></table>{!interviewerQuality.length ? <p className="p-8 text-center text-sm text-slate-500">No synchronized field interviews are available yet.</p> : null}</div>
      </section>
