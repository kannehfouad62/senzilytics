...
export default async function TrainingDashboard() {
  ...
  const [records, competencyMatrix] = await Promise.all([prisma.trainingRecord.findMany({ where: { user: { organizationId } }, include: { user: { include: { department: { include: { site: true } } } } } }), getCompetencyMatrixService(organizationId, now)]);
  ...
  return <div><div className="flex flex-wrap justify-between gap-4"><div><p className="text-sm text-cyan-300">Competency Intelligence</p><h1 className="mt-2 text-4xl font-bold">Training Analytics</h1></div><div className="flex gap-3"><Link href="/training/competencies/matrix" className="h-fit rounded-xl bg-cyan-300 px-4 py-2 font-semibold text-slate-950">Competency Matrix</Link><Link href="/training/requirements" className="h-fit rounded-xl border border-white/10 px-4 py-2">Requirements</Link></div></div><div className="mt-8 grid gap-4 md:grid-cols-3 xl:grid-cols-7">{[["Assignments", total], ["Completion rate", `${total ? Math.round(completed / total * 100) : 0}%`], ["Overdue", overdue], ["Expiring in 30 days", expiring], ["Competency gaps", competencyMatrix.gaps], ["Overdue gaps", competencyMatrix.overdueGaps], ["Critical gaps", competencyMatrix.criticalGaps]].map(([label, value]) => <div key={label} className="rounded-3xl border border-white/10 bg-white/5 p-6"><p className="text-sm text-slate-400">{label}</p><p className="mt-2 text-3xl font-bold">{value}</p></div>)}</div><section className="mt-8 rounded-3xl border border-white/10 bg-white/5 p-6"><h2 className="text-xl font-semibold">Site Compliance Matrix</h2><div className="mt-5 space-y-3">{[...groups.values()].map((group) => <div key={group.name} className="grid grid-cols-4 rounded-xl bg-slate-950/50 p-4"><span>{group.name}</span><span>{group.complete}/{group.total} complete</span><span>{group.total ? Math.round(group.complete / group.total * 100) : 0}%</span><span className={group.overdue ? "text-red-300" : "text-emerald-300"}>{group.overdue} overdue</span></div>)}</div></section></div>;
}
