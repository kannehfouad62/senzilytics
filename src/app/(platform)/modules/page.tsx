...
import { filterUserVisibleModules } from "@/core/navigation/tenant-module-catalog";
import { prisma } from "@/lib/prisma";

export default async function AllModulesPage() {
  await requirePermission(PermissionKey.VIEW_DASHBOARD);
  const [{ organization, user }, permissions] = await Promise.all([getCurrentUserTenant(), getCurrentUserPermissions()]);
  if (!organization) return null;
  const userModuleAssignments = await prisma.userModuleAssignment.findMany({ where: { organizationId: organization.id, userId: user.id }, select: { moduleKey: true, enabled: true } });
  ...
  const sections = [
    { label: "Platform and intelligence", items: primary }, { label: "EHS management", items: filterNavigationItems(ehsNavItems, permissions) },
    { label: "Audit management", items: filterNavigationItems(auditNavItems, permissions) }, { label: "Research and analytics", items: filterNavigationItems(researchNavItems, permissions) },
    { label: "Inspections", items: filterNavigationItems(inspectionNavItems, permissions) }, { label: "Governance", items: governance },
  ].map((section) => ({ ...section, items: filterUserVisibleModules(userModuleAssignments, section.items) }));
  return <div><p className="flex items-center gap-2 text-sm text-cyan-300"><PanelsTopLeft size={18}/>Module directory</p><h1 className="mt-2 text-4xl font-bold">All Modules</h1><p className="mt-2 max-w-3xl text-slate-400">Explore every module available under your role, tenant assignment, and subscription. Your sidebar prioritizes {industryCategoryLabels[organization.industryCategory].toLowerCase()} recommendations.</p>
