          ...
          <tbody>
            {users.map((user) => (
              ...
              >
                ...
                <td className="px-6 py-5">{user.id===currentUser.id?<span className="text-slate-500">{user.isActive?"Active":"Suspended"} · Current account</span>:<form action={setTenantUserActive}><input type="hidden" name="id" value={user.id}/><input type="hidden" name="active" value={user.isActive?"false":"true"}/><button className={user.isActive?"text-emerald-300":"text-red-300"}>{user.isActive?"Active · Suspend":"Suspended · Restore"}</button></form>}</td>
                <td className="px-6 py-5">{user.id===currentUser.id?<span className="text-slate-500">Managed by another owner or super administrator</span>:canAssignModules?<details className="min-w-64"><summary className="cursor-pointer text-cyan-300">Assign modules</summary><form action={setUserModuleAssignments} className="mt-3 space-y-3"><input type="hidden" name="userId" value={user.id}/><div className="grid gap-2">{assignableModules.map(module=>{const inherited=!user.userModuleAssignments.length;const checked=inherited||user.userModuleAssignments.some(assignment=>assignment.moduleKey===module.key&&assignment.enabled);return <label key={module.key} className="flex items-center gap-2 text-xs text-slate-300"><input type="checkbox" name="moduleKeys" value={module.key} defaultChecked={checked} className="accent-cyan-300"/>{module.label}</label>})}</div><button className="rounded-lg bg-cyan-300 px-3 py-2 text-xs font-semibold text-slate-950">Save module access</button></form></details>:<span className="text-slate-500">Owner access required</span>}</td>
              </tr>
            ))}
          </tbody>
        </table>
