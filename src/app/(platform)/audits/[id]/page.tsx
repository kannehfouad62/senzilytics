...
export default async function AuditDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ id }, { organizationId }, canManage, canUseAi] = await Promise.all([params, getCurrentUserTenant(), hasPermission(PermissionKey.MANAGE_AUDITS), hasPermission(PermissionKey.USE_AI)]);
    ...
    <div>
      ...
      <div className="mt-6 flex flex-wrap items-start justify-between gap-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {canManage && startableStatuses.has(audit.status) && <form action={startAuditExecution}><input type="hidden" name="auditId" value={audit.id} /><button className="rounded-2xl bg-cyan-300 px-4 py-2 text-sm font-semibold text-slate-950">Start Execution</button></form>}
          {canManage && audit.status === EnterpriseAuditStatus.IN_PROGRESS && <AuditActionForm action={submitAuditForReviewWithFeedback}><input type="hidden" name="auditId" value={audit.id} /><button className="rounded-2xl bg-amber-300 px-4 py-2 text-sm font-semibold text-slate-950">Submit for Review</button></AuditActionForm>}
          {canManage && audit.status === EnterpriseAuditStatus.PENDING_REVIEW && <form action={completeAudit}><input type="hidden" name="auditId" value={audit.id} /><button className="rounded-2xl bg-emerald-400 px-4 py-2 text-sm font-semibold text-slate-950">Complete Audit</button></form>}
        </div>
      </div>
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[1.4fr_1fr]">
        ...
        <div className="space-y-6">
          ...
          <Panel title="Audit team" icon={<Users size={18} />}>
            ...
            {canManage && <form action={addAuditTeamMember} className="mt-5 space-y-4 rounded-2xl border border-white/10 p-4">
      ...
      <div className="mt-8">
        {canUseAi && <AuditAiAssistant auditId={audit.id} reference={audit.reference} />}
      </div>
      <div className="mt-8">
        <Panel title="Audit Findings, CAPA, and Risk" icon={<CircleAlert size={18} />}>
          {canManage ? <AuditFindingManagement audit={audit} users={users} /> : <Empty text="Finding creation and lifecycle actions are disabled in the read-only demo." />}
        </Panel>
      </div>
    </div>
  );
