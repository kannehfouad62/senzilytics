...
import { EmployeePerformanceGovernance } from "@/features/employee-performance/employee-performance-governance";
import { getEmployeePerformanceGovernance } from "@/modules/employee-performance/employee-performance-governance.service";
import {
...
export default async function EmployeePerformanceReportPage({ params, searchParams }: { params: Promise<{ userId: string }>; searchParams: Promise<{ days?: string }> }) {
  ...
  const canManage = permissions.includes(PermissionKey.MANAGE_EMPLOYEE_PERFORMANCE);
  ...
  const governance = await getEmployeePerformanceGovernance({ organizationId, employeeId: employee.id });
  ...
  return <div>
    ...
    <EmployeePerformanceGovernance governance={governance} employeeId={employee.id} employeeName={employee.name} days={days} canManage={canManage} isEmployee={user.id === employee.id}/>
