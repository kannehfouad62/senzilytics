...
export default async function TenantConsole() {
  ...
  const tenants = await prisma.organization.findMany({ include: { _count: { select: { users: true, sites: true } }, identityProviders: true, onboardingPlan: { select: { status: true, steps: { select: { key: true, status: true } } } } }, orderBy: { name: "asc" } });
  return <div><p className="text-sm text-cyan-300">Platform Administration</p><h1 className="mt-2 text-4xl font-bold">Tenant Provisioning</h1><p className="mt-2 text-slate-400">Create tenants and control their commercial feature entitlements.</p>
    <div className="mt-8 grid gap-6 xl:grid-cols-[.8fr_1.2fr]"><form action={createTenant} className="h-fit space-y-4 rounded-3xl border border-white/10 bg-white/5 p-6"><h2 className="text-xl font-semibold">Create Tenant</h2>
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-cyan-400/10 bg-cyan-400/[.03] p-4"><div><p className="text-xs text-slate-500">Implementation readiness</p><p className="mt-1 text-sm font-semibold text-cyan-200">{tenant.onboardingPlan?`${tenantOnboardingProgress(tenant.onboardingPlan.steps)}% · ${pretty(tenant.onboardingPlan.status)}`:"Not initialized"}</p></div><Link href={`/platform/tenants/${tenant.id}`} className="rounded-xl border border-cyan-400/20 px-4 py-2 text-sm font-semibold text-cyan-200">Open implementation</Link></div>
