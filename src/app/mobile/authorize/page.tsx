...
import { getMobileChallengeService } from "@/modules/mobile/mobile-auth.service";
...
export default async function MobileAuthorizePage({ searchParams }: { searchParams: Promise<{ challenge?: string }> }) {
  ...
  if (!organization || organization.isDemo || !planEntitlements[organization.subscriptionPlan].MOBILE_APPS) return <Message title="Premium mobile access required" detail="Your organization does not currently include native iOS and Android access. Contact your Senzilytics administrator to enable the Premium plan." />;
