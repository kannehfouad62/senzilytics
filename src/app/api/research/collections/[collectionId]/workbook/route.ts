...
export async function GET(request: Request, { params }: { params: Promise<{ collectionId: string }> }) {
  ...
  const filteredData=workbook.addWorksheet("Filtered View");
  filteredData.addRow(["response_id","submitted_at",...variables.map(variable=>variable.key)]);
  for(const row of filteredRows)filteredData.addRow([row.responseId,row.submittedAt,...variables.map(variable=>safe(row.values[variable.key]))]);
  ...
  advanced.addRows([["Advanced analytical output"], ["Method", method], ["X variable", x?.label ?? ""], ["Y variable", y?.label ?? ""], ["Analytical population", filteredRows.length]]);
  if (x) { advanced.addRow([]); advanced.addRow(["Result path", "Value"]); for (const [path, value] of flatten(buildAnalysisSnapshot(method, filteredRows, x, y))) advanced.addRow([path, safe(value)]); }
