...
export async function GET(_request:Request,{params}:{params:Promise<{analysisId:string}>}){
  ...
  const governance:SlideElement[]=[{x:.7,y:.5,w:11,h:.5,text:"Governance & Interpretation",size:26,bold:true},{x:.8,y:1.5,w:11,h:.5,text:`Source: ${source}`,size:15,color:"CBD5E1"},{x:.8,y:2.1,w:11,h:.5,text:`Analyst: ${analysis.analyst.name??"Assigned analyst"}`,size:16,color:"CBD5E1"},{x:.8,y:2.7,w:11,h:.5,text:`Approved by: ${analysis.approvedBy?.name??"Pending independent approval"}`,size:16,color:"CBD5E1"},{x:.8,y:3.4,w:11,h:1,text:`Hypothesis: ${analysis.hypothesis??"Not recorded"}`,size:14,color:"CBD5E1"},{x:.8,y:4.6,w:11,h:1.2,text:`Methodology: ${analysis.methodologyNotes??"Not recorded"}`,size:14,color:"CBD5E1"},{x:.8,y:6.2,w:11,h:.4,text:"Model outputs require qualified review of assumptions, diagnostics, bias and practical significance.",size:10,color:"64748B"}];
