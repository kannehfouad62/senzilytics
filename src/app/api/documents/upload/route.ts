...
async function validateRelatedEntity(input: {
  ...
  switch (input.entityType) {
    ...
    case DocumentEntityType.EMERGENCY_PREPAREDNESS: {
      const record = await prisma.emergencyPlan.findFirst({ where: { id: input.entityId, organizationId: input.organizationId }, select: { id: true } });
      return Boolean(record);
    }

    case DocumentEntityType.BUSINESS_CONTINUITY: {
      const record = await prisma.businessContinuityPlan.findFirst({ where: { id: input.entityId, organizationId: input.organizationId }, select: { id: true } });
      return Boolean(record);
    }
