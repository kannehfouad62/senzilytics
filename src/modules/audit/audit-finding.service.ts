...
export async function createAuditFindingService(input: {
  ...
  const standardClause = input.standardClause ?? question?.standardClause;
  const previousFinding = await prisma.enterpriseAuditFinding.findFirst({ where: { organizationId: input.organizationId, auditId: { not: audit.id }, category: input.category, OR: [...(standardClause ? [{ standardClause }] : []), { title: { equals: input.title, mode: "insensitive" } }] }, select: { reference: true, recurrenceCount: true }, orderBy: { createdAt: "desc" } });
  const reference = `AF-${audit.reference}-${crypto.randomUUID().slice(0, 6).toUpperCase()}`;
  const finding = await prisma.enterpriseAuditFinding.create({ data: { organizationId: input.organizationId, auditId: audit.id, questionId: question?.id, reference, title: input.title, findingType: input.findingType, category: input.category, severity: input.severity, status: EnterpriseAuditFindingStatus.OPEN, description: input.description, objectiveEvidence: input.objectiveEvidence, standardClause, regulatoryRef: input.regulatoryRef ?? question?.regulatoryRef, immediateCorrection: input.immediateCorrection, containmentAction: input.containmentAction, ownerId: owner?.id, dueDate: input.dueDate, isRepeatFinding: Boolean(previousFinding), previousFindingReference: previousFinding?.reference, recurrenceCount: previousFinding ? previousFinding.recurrenceCount + 1 : 0, requiresCapa: input.requiresCapa, requiresRiskReview: input.requiresRiskReview, capaSuggestedAt: input.requiresCapa ? new Date() : null, riskSuggestedAt: input.requiresRiskReview ? new Date() : null, submittedAt: new Date(), createdById: input.userId, updatedById: input.userId } });
...
export async function proposeCapaFromAuditFindingService(input: { organizationId: string; userId: string; auditId: string; findingId: string; title: string; description?: string | null; assignedToId: string; dueDate: Date }) {
  const [finding, owner] = await Promise.all([findTenantFinding(input.findingId, input.auditId, input.organizationId), prisma.user.findFirst({ where: { id: input.assignedToId, organizationId: input.organizationId }, select: { id: true } })]);
  if (!owner) throw new Error("The suggested corrective-action owner is invalid."); if (input.dueDate <= new Date()) throw new Error("Corrective-action due date must be in the future.");
  if (finding.correctiveActionLinks.some((link) => link.status === EnterpriseAuditLinkStatus.PROPOSED || link.correctiveActionId)) throw new Error("This finding already has a pending or created CAPA link.");
  const proposal = await prisma.enterpriseAuditFindingActionLink.create({ data: { findingId: finding.id, status: EnterpriseAuditLinkStatus.PROPOSED, recommendationTitle: input.title, recommendationDescription: input.description, suggestedOwnerId: owner.id, suggestedDueDate: input.dueDate, rationale: `CAPA proposed from ${finding.reference}.`, createdById: input.userId } });
  await prisma.$transaction([prisma.enterpriseAuditFinding.update({ where: { id: finding.id }, data: { requiresCapa: true, capaSuggestedAt: new Date(), updatedById: input.userId } }), prisma.enterpriseAuditFindingHistory.create({ data: { findingId: finding.id, userId: input.userId, action: EnterpriseAuditHistoryAction.FINDING_UPDATED, title: "CAPA recommendation proposed", description: input.title } })]); return proposal;
}

export async function reviewCapaProposalService(input: { organizationId: string; userId: string; auditId: string; findingId: string; linkId: string; approve: boolean }) {
  const finding = await findTenantFinding(input.findingId, input.auditId, input.organizationId); const link = finding.correctiveActionLinks.find((item) => item.id === input.linkId);
  if (!link || link.status !== EnterpriseAuditLinkStatus.PROPOSED) throw new Error("Pending CAPA recommendation not found.");
  if (link.createdById === input.userId) throw new Error("A CAPA recommendation must be reviewed by a different authorized user.");
  if (!input.approve) { await prisma.$transaction([prisma.enterpriseAuditFindingActionLink.update({ where: { id: link.id }, data: { status: EnterpriseAuditLinkStatus.REJECTED, reviewedById: input.userId, reviewedAt: new Date() } }), prisma.enterpriseAuditFindingHistory.create({ data: { findingId: finding.id, userId: input.userId, action: EnterpriseAuditHistoryAction.FINDING_UPDATED, title: "CAPA recommendation rejected", description: link.recommendationTitle } })]); return null; }
  if (!link.recommendationTitle || !link.suggestedOwnerId || !link.suggestedDueDate) throw new Error("The CAPA recommendation is incomplete.");
  const [owner, creator] = await Promise.all([prisma.user.findFirst({ where: { id: link.suggestedOwnerId, organizationId: input.organizationId } }), prisma.user.findFirst({ where: { id: input.userId, organizationId: input.organizationId } })]); if (!owner || !creator) throw new Error("CAPA owner or reviewer is invalid.");
  const action = await prisma.$transaction(async (tx) => { const correctiveAction = await tx.correctiveAction.create({ data: { title: link.recommendationTitle!, description: link.recommendationDescription, status: Status.OPEN, riskLevel: severityToRiskLevel(finding.severity), dueDate: link.suggestedDueDate!, assignedToId: owner.id } }); await tx.enterpriseAuditFindingActionLink.update({ where: { id: link.id }, data: { correctiveActionId: correctiveAction.id, status: EnterpriseAuditLinkStatus.CREATED, reviewedById: input.userId, reviewedAt: new Date() } }); await tx.enterpriseAuditFinding.update({ where: { id: finding.id }, data: { status: finding.status === EnterpriseAuditFindingStatus.OPEN ? EnterpriseAuditFindingStatus.ACTION_REQUIRED : finding.status } }); await tx.enterpriseAuditFindingHistory.create({ data: { findingId: finding.id, userId: input.userId, action: EnterpriseAuditHistoryAction.CAPA_LINKED, title: "CAPA recommendation approved and created", description: correctiveAction.title } }); return correctiveAction; });
  await createNotification({ organizationId: input.organizationId, userId: owner.id, type: NotificationType.ASSIGNMENT, title: "Audit corrective action assigned", message: `Approved corrective action from ${finding.reference}: ${action.title}`, link: `/audits/${finding.auditId}` });
  if (owner.email) await sendCorrectiveActionAssignmentEmail({ recipientEmail: owner.email, recipientName: owner.name, actionId: action.id, actionTitle: action.title, actionDescription: action.description, incidentId: null, incidentTitle: `Audit: ${finding.audit.title}`, dueDate: action.dueDate, riskLevel: action.riskLevel, assignedByName: creator.name }).catch(() => undefined); return action;
}

export async function proposeRiskFromAuditFindingService(input: { organizationId: string; userId: string; auditId: string; findingId: string; title: string; description: string; category: RiskCategory; hazardType?: string | null; ownerId?: string | null; likelihood: RiskLikelihood; impact: RiskImpact; nextReviewDate?: Date | null }) {
  const finding = await findTenantFinding(input.findingId, input.auditId, input.organizationId); if (input.ownerId && !(await prisma.user.findFirst({ where: { id: input.ownerId, organizationId: input.organizationId }, select: { id: true } }))) throw new Error("The suggested risk owner is invalid.");
  if (finding.riskLinks.some((link) => link.status === EnterpriseAuditLinkStatus.PROPOSED || link.riskId)) throw new Error("This finding already has a pending or created Risk link.");
  const proposal = await prisma.enterpriseAuditFindingRiskLink.create({ data: { findingId: finding.id, status: EnterpriseAuditLinkStatus.PROPOSED, proposedRiskTitle: input.title, proposedRiskDescription: input.description, proposedHazard: input.hazardType, proposedLikelihood: input.likelihood, proposedImpact: input.impact, rationale: JSON.stringify({ category: input.category, ownerId: input.ownerId ?? null, nextReviewDate: input.nextReviewDate?.toISOString() ?? null }), createdById: input.userId } });
  await prisma.$transaction([prisma.enterpriseAuditFinding.update({ where: { id: finding.id }, data: { requiresRiskReview: true, riskSuggestedAt: new Date(), updatedById: input.userId } }), prisma.enterpriseAuditFindingHistory.create({ data: { findingId: finding.id, userId: input.userId, action: EnterpriseAuditHistoryAction.FINDING_UPDATED, title: "Risk recommendation proposed", description: input.title } })]); return proposal;
}

export async function reviewRiskProposalService(input: { organizationId: string; userId: string; auditId: string; findingId: string; linkId: string; approve: boolean }) {
  const finding = await findTenantFinding(input.findingId, input.auditId, input.organizationId); const link = finding.riskLinks.find((item) => item.id === input.linkId); if (!link || link.status !== EnterpriseAuditLinkStatus.PROPOSED) throw new Error("Pending Risk recommendation not found.");
  if (link.createdById === input.userId) throw new Error("A Risk recommendation must be reviewed by a different authorized user.");
  if (!input.approve) { await prisma.$transaction([prisma.enterpriseAuditFindingRiskLink.update({ where: { id: link.id }, data: { status: EnterpriseAuditLinkStatus.REJECTED, reviewedById: input.userId, reviewedAt: new Date() } }), prisma.enterpriseAuditFindingHistory.create({ data: { findingId: finding.id, userId: input.userId, action: EnterpriseAuditHistoryAction.FINDING_UPDATED, title: "Risk recommendation rejected", description: link.proposedRiskTitle } })]); return null; }
  if (!link.proposedRiskTitle || !link.proposedRiskDescription || !link.proposedLikelihood || !link.proposedImpact) throw new Error("The Risk recommendation is incomplete."); let metadata: { category?: RiskCategory; ownerId?: string | null; nextReviewDate?: string | null } = {}; try { metadata = JSON.parse(link.rationale || "{}"); } catch { metadata = {}; }
  const risk = await createRiskService({ organizationId: input.organizationId, userId: input.userId, title: link.proposedRiskTitle, description: link.proposedRiskDescription, category: metadata.category && Object.values(RiskCategory).includes(metadata.category) ? metadata.category : RiskCategory.OTHER, hazardType: link.proposedHazard, process: `Enterprise Audit ${finding.audit.reference}`, siteId: finding.audit.siteId, departmentId: finding.audit.departmentId, ownerId: metadata.ownerId, initialLikelihood: link.proposedLikelihood, initialImpact: link.proposedImpact, currentLikelihood: link.proposedLikelihood, currentImpact: link.proposedImpact, residualLikelihood: link.proposedLikelihood, residualImpact: link.proposedImpact, reviewFrequency: RiskReviewFrequency.ANNUAL, nextReviewDate: metadata.nextReviewDate ? new Date(metadata.nextReviewDate) : null });
  await prisma.$transaction([prisma.enterpriseAuditFindingRiskLink.update({ where: { id: link.id }, data: { riskId: risk.id, status: EnterpriseAuditLinkStatus.CREATED, reviewedById: input.userId, reviewedAt: new Date() } }), prisma.enterpriseAuditFindingHistory.create({ data: { findingId: finding.id, userId: input.userId, action: EnterpriseAuditHistoryAction.RISK_LINKED, title: "Risk recommendation approved and created", description: `${risk.reference}: ${risk.title}` } })]); return risk;
}
