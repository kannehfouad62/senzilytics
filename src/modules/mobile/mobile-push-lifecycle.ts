...
export function mobilePushIdentityMatches(
  delivery: { organizationId: string; userId: string },
  token: { organizationId: string; userId: string },
) {
  return delivery.organizationId === token.organizationId && delivery.userId === token.userId;
}
