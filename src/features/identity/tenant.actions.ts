...
import { PermissionKey, TenantInvitationStatus, UserRole } from "@prisma/client";
...
export async function createTenant(d:FormData){const admin=await requirePlatformAdministrator();const name=value(d,"name"),email=value(d,"adminEmail").toLowerCase(),domain=value(d,"domain").toLowerCase();const organization=await prisma.organization.create({data:{name,industry:String(d.get("industry")||"").trim()||null,allowedEmailDomains:[domain]}});await sendInvitation({organizationId:organization.id,invitedById:admin.id,email,name:value(d,"adminName"),role:UserRole.ORG_ADMIN,departmentId:null});revalidatePath("/platform/tenants")}
