...
export function ResearchQuestionnaireForm({projectId,consentRequired,templates}:{projectId:string;consentRequired:boolean;templates:Array<{id:string;version:number;name:string;projectLabel:string;fieldCount:number}>}) {
  const [state, action, pending] = useActionState(createResearchQuestionnaire, initialFormActionState);
  useRefreshOnSuccess(state);
  return <form action={action} className="rounded-3xl border border-white/10 bg-white/[.04] p-6"><input type="hidden" name="projectId" value={projectId}/><div className="grid gap-5 md:grid-cols-2"><Field label="Questionnaire name"><input name="name" required maxLength={160} className={field}/></Field><Field label="Respondent identity mode"><select name="identityMode" defaultValue={ResearchResponseIdentityMode.PSEUDONYMIZED} className={field}>{Object.values(ResearchResponseIdentityMode).map(value=><option value={value} key={value}>{pretty(value)}</option>)}</select></Field><Field label="Target audience"><input name="targetAudience" maxLength={500} className={field}/></Field><Field label="Default language code"><input name="defaultLanguage" defaultValue="en" maxLength={12} className={field}/></Field></div><Field label="Start from a governed questionnaire template (optional)"><select name="templateVersionId" defaultValue="" className={field}><option value="">Blank questionnaire</option>{templates.map(template=><option key={template.id} value={template.id}>{template.name} v{template.version} — {template.fieldCount} fields — {template.projectLabel}</option>)}</select></Field><Field label="Questionnaire purpose"><textarea name="purpose" required minLength={10} maxLength={3000} rows={4} className={field}/></Field><Field label={`Participant consent statement${consentRequired?" — required":""}`}><textarea name="consentStatement" required={consentRequired} maxLength={6000} rows={5} className={field}/></Field><p className="mt-4 text-sm text-slate-500">A controlled Form Studio draft will be created and linked to this project. A selected template copies its published fields as an independent snapshot; published sources remain immutable.</p><button disabled={pending} className={`mt-5 ${button}`}>{pending?"Creating…":"Create and open Questionnaire Studio"}</button><Feedback state={state}/></form>;
}
