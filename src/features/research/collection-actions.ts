...
export async function assignQuestionnaireRespondent(_state:FormActionState,data:FormData):Promise<FormActionState>{
  ...
  try{await assignResearchRespondent({organizationId,actorId:user.id,collectionId,respondentId:required(data,"respondentId"),dueAt:date(data,"dueAt")});revalidatePath(`/research/collections/${collectionId}`);revalidatePath("/research/my-questionnaires");return{status:"SUCCESS",message:"Respondent assigned and notified."}}catch(cause){return failure(cause,"Respondent could not be assigned.")}
}

export async function changeCollectionStatus(_state:FormActionState,data:FormData):Promise<FormActionState>{
  await requirePermission(PermissionKey.MANAGE_RESEARCH_DATASETS);const{organizationId,user}=await getCurrentUserTenant();const collectionId=required(data,"collectionId");
  try{const raw=required(data,"status");if(!Object.values(ResearchCollectionStatus).includes(raw as ResearchCollectionStatus))throw new Error("Select a valid collection status.");await setResearchCollectionStatus({organizationId,actorId:user.id,collectionId,status:raw as ResearchCollectionStatus});revalidatePath(`/research/collections/${collectionId}`);revalidatePath("/research/my-questionnaires");return{status:"SUCCESS",message:"Collection status updated."}}catch(cause){return failure(cause,"Collection status could not be updated.")}
}
...
export async function submitResearchResponse(_state:FormActionState,data:FormData):Promise<FormActionState>{
  ...
  try{await submitAssignedResearchQuestionnaire({organizationId,userId:user.id,assignmentId,consent:data.get("participantConsent")==="on",data});revalidatePath("/research/my-questionnaires");revalidatePath(`/research/my-questionnaires/${assignmentId}`)}catch(cause){return failure(cause,"Questionnaire response could not be submitted.")}
