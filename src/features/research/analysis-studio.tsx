...
import { Bar, BarChart, CartesianGrid, Funnel, FunnelChart, LabelList, Line, LineChart, PolarAngleAxis, PolarGrid, PolarRadiusAxis, Radar, RadarChart, ResponsiveContainer, Sankey, Scatter, ScatterChart, Tooltip, XAxis, YAxis } from "recharts";
...
import { applyResearchFilters, buildFunnelData, buildGeographicPoints, buildHeatmapData, buildRadarData, buildSankeyData, buildSmallMultipleData, researchFilterOptions, type ResearchFilterClause, type ResearchFilterDefinition, type ResearchFilterOperator } from "@/modules/research/research-visualization";

type AnalysisMode = "AUTO" | "DISTRIBUTION" | "BOX_PLOT" | "CROSSTAB" | "CORRELATION" | "GROUP_COMPARISON" | "REGRESSION" | "NON_PARAMETRIC" | "ASSUMPTIONS";
type VisualizationType="AUTO"|"BAR"|"LINE"|"FUNNEL"|"RADAR"|"HEATMAP"|"SMALL_MULTIPLES"|"SANKEY"|"GEOGRAPHIC";
...
export function ResearchAnalysisStudio({ variables, rows, collectionId, datasetVersionId }: { variables: ResearchVariable[]; rows: ResearchDataRow[]; collectionId?: string; datasetVersionId?: string }) {
  ...
  const [filters, setFilters] = useState<ResearchFilterClause[]>([]);
  ...
  return <div className="space-y-6">
    ...
    <FilterCanvas variables={variables} rows={rows} filters={filters} logic={filterLogic} onLogic={setFilterLogic} onChange={setFilters} included={filtered.length}/>
...
function AnalysisVisualization({ mode, visualizationType, rows, x, y, bins, showGrid, expanded }: { mode: AnalysisMode; visualizationType:VisualizationType; rows: ResearchDataRow[]; x: ResearchVariable | null; y: ResearchVariable | null; bins: number; showGrid: boolean; expanded: boolean }) {
  ...
  if((mode==="NON_PARAMETRIC"||mode==="ASSUMPTIONS")&&x.type==="NUMBER"){
    if(!y)return <Bars data={histogram(rows,x,bins)} showGrid={showGrid} expanded={expanded}/>;
    if(y.type==="NUMBER"){const points=buildChartData(rows,x,y).filter((point):point is{x:number;y:number}=>"x" in point&&"y" in point);return <div className={expanded?"h-[36rem]":"h-96"}><ResponsiveContainer width="100%" height="100%"><ScatterChart>{showGrid&&<CartesianGrid stroke="#1e293b"/>}<XAxis dataKey="x" name={x.label} stroke="#94a3b8"/><YAxis dataKey="y" name={y.label} stroke="#94a3b8"/><Tooltip/><Scatter data={points} fill="#fbbf24"/></ScatterChart></ResponsiveContainer></div>}
  }
  if (mode === "CROSSTAB") {
  ...
  if (mode === "CORRELATION" || mode === "REGRESSION") {
    ...
    if (mode === "REGRESSION") {
      ...
      return <div className={expanded?"h-[36rem]":"h-96"}><ResponsiveContainer width="100%" height="100%"><LineChart>{showGrid&&<CartesianGrid stroke="#1e293b"/>}<XAxis type="number" dataKey="x" domain={["auto", "auto"]} stroke="#94a3b8"/><YAxis type="number" dataKey="y" domain={["auto", "auto"]} stroke="#94a3b8"/><Tooltip/><Line data={line} dataKey="y" stroke="#a78bfa" dot={false} strokeWidth={2}/><Line data={points} dataKey="y" stroke="transparent" dot={{ fill: "#22d3ee", r: 4 }}/></LineChart></ResponsiveContainer></div>;
    }
    return <div className={expanded?"h-[36rem]":"h-96"}><ResponsiveContainer width="100%" height="100%"><ScatterChart>{showGrid&&<CartesianGrid stroke="#1e293b"/>}<XAxis dataKey="x" name={x.label} stroke="#94a3b8"/><YAxis dataKey="y" name={y.label} stroke="#94a3b8"/><Tooltip cursor={{ strokeDasharray: "3 3" }}/><Scatter data={points} fill="#22d3ee"/></ScatterChart></ResponsiveContainer></div>;
  }
...
function AdvancedVisualization({type,rows,x,y,showGrid,expanded}:{type:Exclude<VisualizationType,"AUTO">;rows:ResearchDataRow[];x:ResearchVariable;y:ResearchVariable|null;showGrid:boolean;expanded:boolean}){
  ...
  if(type==="SANKEY"){if(!y||x.type==="NUMBER"||y.type==="NUMBER")return <Empty text="Sankey flow requires categorical source (X) and target (Y) variables."/>;const data=buildSankeyData(rows,x,y);return data.links.length?<div className={height}><ResponsiveContainer width="100%" height="100%"><Sankey data={data} nodePadding={28} nodeWidth={12} link={{stroke:"#22d3ee",strokeOpacity:.28}} node={{fill:"#a78bfa",stroke:"#c4b5fd"}}><Tooltip/></Sankey></ResponsiveContainer></div>:<Empty text="No source-to-target flows are available."/>}
  if(type==="GEOGRAPHIC"){if(x.type!=="NUMBER"||y?.type!=="NUMBER")return <Empty text="Geographic coordinates require numeric longitude (X) and latitude (Y) variables."/>;const points=buildGeographicPoints(rows,x,y);return points.length?<div className={height}><ResponsiveContainer width="100%" height="100%"><ScatterChart margin={{top:18,right:24,bottom:24,left:12}}>{showGrid&&<CartesianGrid stroke="#1e293b" strokeDasharray="3 3"/>}<XAxis type="number" dataKey="longitude" domain={[-180,180]} ticks={[-180,-120,-60,0,60,120,180]} name="Longitude" unit="°" stroke="#94a3b8"/><YAxis type="number" dataKey="latitude" domain={[-90,90]} ticks={[-90,-60,-30,0,30,60,90]} name="Latitude" unit="°" stroke="#94a3b8"/><Tooltip cursor={{strokeDasharray:"3 3"}}/><Scatter data={points} name="Research location" fill="#22d3ee"/></ScatterChart></ResponsiveContainer><p className="-mt-5 text-center text-xs text-slate-500">Validated WGS84-style coordinates · longitude −180–180 · latitude −90–90</p></div>:<Empty text="No valid longitude and latitude pairs are available."/>}
...
function FilterCanvas({ variables, rows, filters, logic, onLogic, onChange, included }: { variables: ResearchVariable[]; rows: ResearchDataRow[]; filters: ResearchFilterClause[]; logic: "ALL"|"ANY"; onLogic: (value:"ALL"|"ANY")=>void; onChange:(value:ResearchFilterClause[])=>void; included:number }) {
  const update=(id:string,patch:Partial<ResearchFilterClause>)=>onChange(filters.map(filter=>filter.id===id?{...filter,...patch}:filter));
  const add=()=>{const variable=variables[0];if(!variable||filters.length>=8)return;onChange([...filters,{id:`filter-${Date.now()}`,variableKey:variable.key,operator:variable.type==="NUMBER"?"RANGE":"EQUALS",value:""}])};
  return <section className={`${panel} p-5`}>
    <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-cyan-300">Cross-filter canvas</p><h2 className="mt-1 text-lg font-semibold">Dashboard filters</h2><p className="mt-1 text-xs text-slate-400">Combine up to eight filters. Every chart, statistic, weight, and saved snapshot uses the same included population.</p></div><div className="flex items-center gap-2"><span className="rounded-full border border-emerald-400/20 bg-emerald-400/5 px-3 py-1.5 text-xs text-emerald-300">{included} of {rows.length} included</span><button type="button" onClick={add} disabled={!variables.length||filters.length>=8} className="rounded-xl bg-cyan-300 px-4 py-2 text-sm font-semibold text-slate-950 disabled:opacity-40">Add filter</button></div></div>
    {filters.length>1&&<label className="mt-4 flex items-center gap-2 text-xs text-slate-400"><span>Match</span><select value={logic} onChange={event=>onLogic(event.target.value as "ALL"|"ANY")} className={input}><option value="ALL">All conditions (AND)</option><option value="ANY">Any condition (OR)</option></select></label>}
    <div className="mt-4 grid gap-3">{filters.map((filter,index)=>{const variable=variables.find(item=>item.key===filter.variableKey)??variables[0],options=variable?researchFilterOptions(rows,variable.key):[],operatorOptions:ResearchFilterOperator[]=variable?.type==="NUMBER"?["RANGE","EQUALS","NOT_EQUALS","IS_MISSING","IS_NOT_MISSING"]:["EQUALS","NOT_EQUALS","CONTAINS","IS_MISSING","IS_NOT_MISSING"];return <div key={filter.id} className="grid gap-3 rounded-2xl border border-white/10 bg-slate-950/40 p-4 lg:grid-cols-[auto_1fr_1fr_1.4fr_auto] lg:items-end"><span className="pb-2 text-xs font-semibold text-slate-500">{index+1}</span><label className="grid gap-1 text-xs text-slate-400"><span>Variable</span><select value={filter.variableKey} onChange={event=>{const next=variables.find(item=>item.key===event.target.value);update(filter.id,{variableKey:event.target.value,operator:next?.type==="NUMBER"?"RANGE":"EQUALS",value:"",minimum:undefined,maximum:undefined})}} className={input}>{variables.map(item=><option key={item.key} value={item.key}>{item.label}</option>)}</select></label><label className="grid gap-1 text-xs text-slate-400"><span>Condition</span><select value={filter.operator} onChange={event=>update(filter.id,{operator:event.target.value as ResearchFilterOperator})} className={input}>{operatorOptions.map(operator=><option key={operator} value={operator}>{operator.replaceAll("_"," ").toLocaleLowerCase()}</option>)}</select></label><FilterValueEditor filter={filter} variable={variable} options={options} onUpdate={patch=>update(filter.id,patch)}/><button type="button" onClick={()=>onChange(filters.filter(item=>item.id!==filter.id))} className="rounded-xl border border-red-400/20 px-3 py-2 text-sm text-red-300">Remove</button></div>})}</div>
    {!filters.length&&<div className="mt-4 rounded-2xl border border-dashed border-white/10 p-6 text-center text-sm text-slate-500">No filters applied. The canvas includes the complete governed dataset.</div>}
  </section>;
}

function FilterValueEditor({ filter, variable, options, onUpdate }: { filter:ResearchFilterClause; variable?:ResearchVariable; options:string[]; onUpdate:(patch:Partial<ResearchFilterClause>)=>void }) {
  if(["IS_MISSING","IS_NOT_MISSING"].includes(filter.operator))return <p className="pb-2 text-xs text-slate-500">No value required</p>;
  if(filter.operator==="RANGE")return <div className="grid grid-cols-2 gap-2"><label className="grid gap-1 text-xs text-slate-400"><span>Minimum</span><input type="number" value={filter.minimum??""} onChange={event=>onUpdate({minimum:event.target.value===""?undefined:Number(event.target.value)})} className={input}/></label><label className="grid gap-1 text-xs text-slate-400"><span>Maximum</span><input type="number" value={filter.maximum??""} onChange={event=>onUpdate({maximum:event.target.value===""?undefined:Number(event.target.value)})} className={input}/></label></div>;
  if(filter.operator!=="CONTAINS"&&variable?.type!=="NUMBER")return <label className="grid gap-1 text-xs text-slate-400"><span>Value</span><select value={filter.value??""} onChange={event=>onUpdate({value:event.target.value})} className={input}><option value="">Select value</option>{options.map(option=><option key={option} value={option}>{option}</option>)}</select></label>;
  return <label className="grid gap-1 text-xs text-slate-400"><span>Value</span><input type={variable?.type==="NUMBER"?"number":"text"} value={filter.value??""} onChange={event=>onUpdate({value:event.target.value})} className={input}/></label>;
}

function Bars({ data, showGrid=true, expanded=false }: { data: Array<Record<string, string | number>>; showGrid?: boolean; expanded?: boolean }) { return <div className={expanded?"h-[36rem]":"h-96"}><ResponsiveContainer width="100%" height="100%"><BarChart data={data}>{showGrid&&<CartesianGrid stroke="#1e293b"/>}<XAxis dataKey="name" stroke="#94a3b8"/><YAxis stroke="#94a3b8"/><Tooltip/><Bar dataKey="value" fill="#22d3ee" radius={[7, 7, 0, 0]}/></BarChart></ResponsiveContainer></div>; }
