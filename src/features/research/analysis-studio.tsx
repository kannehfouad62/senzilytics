...
export function ResearchAnalysisStudio({ variables, rows, collectionId }: { variables: ResearchVariable[]; rows: ResearchDataRow[]; collectionId: string }) {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-5 xl:grid-cols-[.32fr_1fr]">
      ...
      <div className="space-y-5">
        ...
        <section className={`${panel} p-6`}>
          <div className="flex flex-wrap justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-cyan-300">{modes.find(item => item.value === effectiveMode)?.label}</p><h2 className="mt-1 text-xl font-semibold">{analysisTitle(effectiveMode, x, y)}</h2><p className="mt-1 text-sm text-slate-400">{methodRequirement(effectiveMode)}</p></div><div className="flex gap-2"><a href={`/api/research/collections/${collectionId}/workbook?${query}`} className="rounded-xl border border-emerald-400/25 px-4 py-2 text-sm text-emerald-300">Excel workbook</a><a href={`/api/research/collections/${collectionId}/presentation?${query}`} className="rounded-xl border border-violet-400/25 px-4 py-2 text-sm text-violet-300">PowerPoint</a></div></div>
    ...
    {x && <form action={saveAction} className={`${panel} p-6`}>
      <input type="hidden" name="collectionId" value={collectionId}/><input type="hidden" name="method" value={effectiveMode}/><input type="hidden" name="xVariableKey" value={x.key}/><input type="hidden" name="yVariableKey" value={y?.key ?? ""}/><input type="hidden" name="filterVariableKey" value={filterKey}/><input type="hidden" name="filterValue" value={filterValue}/>
