...
export async function saveResearchAnalysis(_state: FormActionState, data: FormData): Promise<FormActionState> {
  ...
  try {
    ...
    const rawFilterDefinition = text(data, "filterDefinition", 12000);
    const rawVisualizationConfig = text(data, "visualizationConfig", 4000);
    ...
    let parsedVisualization: unknown = {};
    try { parsedFilters = rawFilterDefinition ? JSON.parse(rawFilterDefinition) : {}; } catch { throw new Error("The dashboard filter configuration is invalid."); }
    try { parsedVisualization = rawVisualizationConfig ? JSON.parse(rawVisualizationConfig) : {}; } catch { throw new Error("The visualization configuration is invalid."); }
    const filterDefinition = normalizeResearchFilters(parsedFilters, dataset.variables);
    const visualizationConfig = parsedVisualization && typeof parsedVisualization === "object" ? parsedVisualization : {};
    const rows = filterDefinition.clauses.length ? applyResearchFilters(analysisRows, filterDefinition) : filterVariableKey && filterValue ? analysisRows.filter(row => categories(row.values[filterVariableKey]).includes(filterValue)) : analysisRows;
    ...
    const analysis = await prisma.researchAnalysis.create({ data: {
      ...
      filterVariableKey,
      filterValue,
      filterDefinition: JSON.parse(JSON.stringify(filterDefinition)),
      visualizationConfig: JSON.parse(JSON.stringify(visualizationConfig)),
      weightVariableKey,
...
export async function changeResearchAnalysisStatus(_state: FormActionState, data: FormData): Promise<FormActionState> {
  ...
  try {
    ...
    if (target === ResearchAnalysisStatus.APPROVED) {
      ...
      const currentFilters = normalizeResearchFilters(analysis.filterDefinition, currentDataset?.variables ?? []);
      const currentPopulation = currentFilters.clauses.length ? applyResearchFilters(currentRows, currentFilters).length : analysis.filterVariableKey && analysis.filterValue ? currentRows.filter(row => categories(row.values[analysis.filterVariableKey!]).includes(analysis.filterValue!)).length : currentRows.length;
